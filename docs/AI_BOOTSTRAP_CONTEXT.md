...
---
# Authoritative architecture constraints for pricing + workflow system
...
4. If no AdminSettings record exists:
   - calculateQuote / calculateQuoteOnly must fail
   - checkReleaseReadiness must return releaseReady=false
...
9. Partial or malformed AdminSettings must NOT trigger field-level hardcoded fallback in production.
10. Missing required config fields must block readiness and pricing rather than silently substituting defaults.
...
The pricing engine must remain deterministic and versioned.

MonthlyPrice must be computed from:
- configured base pricing
- configured additive tokens
- configured risk adjustments
- configured frequency logic

The exact numeric targets are not hardcoded in this bootstrap.
They must come from AdminSettings.

The platform may support:
- weekly pricing
- twice-weekly pricing
- one-time recovery / green-to-clean pricing
- price ranges for uncertain pool size / uncertain inspection inputs

But all pricing behavior must remain:
- deterministic
- auditable
- versioned
- reproducible from stored inputs + stored config context

------------------------------------------------------------------
PRICING CONFIG INVARIANTS
------------------------------------------------------------------

AdminSettings must define, at minimum:
- size-tier/base pricing structure
- additive token definitions
- risk bracket definitions
- frequency escalation rules
- green-to-clean / recovery pricing rules
- pricing floor / sanity validations
- pricing engine version metadata

If any required pricing section is missing, malformed, or unreadable:
- releaseReady must be false
- pricing must not silently continue with fallback values

Customer-facing pricing must never expose internal config metadata.

------------------------------------------------------------------
VERSIONING
------------------------------------------------------------------

PRICING_ENGINE_VERSION must be explicit and versioned.
...
- pricing config parseable
- required pricing sections present
- token definitions valid
- risk structure valid
- frequency escalation logic valid
...
Rule:
- usingDefaults must be false in production.
- If defaults would be used, set releaseReady=false and add blocker DEFAULTS_NOT_ALLOWED.
...
- Quotes must be stateless and regenerable from stored inputs + stored config context.
- Quotes should expire (recommended 14 days).
- Customer-visible quote behavior must not depend on hidden frontend-only calculations.
- Do not duplicate pricing logic in the frontend.

------------------------------------------------------------------
INSPECTION SCHEDULING + CALENDAR + PIPELINE (2026-03 Stabilization)
------------------------------------------------------------------

This section governs lead stage progression, inspection scheduling, calendar events, rescheduling, and deletion behavior.
It MUST NOT modify pricing engine math or chemistry invariants.
...
- Side effects must be idempotent where applicable.
...
Scheduling and reschedule approval must ensure:
- only one active inspection appointment remains for a lead
- duplicate active inspection events are cancelled
- cancelled events are marked with explicit cancelReason
- active appointment state remains aligned across InspectionRecord, CalendarEvent, and Lead mirrors

On changed appointment requests, the system may:
- update the active appointment in place, OR
- create a replacement appointment and cancel the previous active one
...
scheduleFirstInspectionPublicV2
- resolves token inline
- creates/ensures InspectionRecord
- creates/updates CalendarEvent projection
- syncs Lead mirror fields
- enforces duplicate protection
- updates pipeline stage inline
- sends inspection confirmation email inline
...
- approval updates InspectionRecord (authoritative) → CalendarEvent (projection) → Lead mirrors
- must remain idempotent

softDeleteLeadV2
- soft-deletes Lead
- cancels all linked InspectionRecords and CalendarEvents with reason lead_deleted
...
Any attempt to set Lead.rls.update = true is a security defect:
LEAD_UPDATE_RLS_TOO_PERMISSIVE
...
Optional:
- an admin-only toggle “Show cancelled” for audit
...
- The public quote flow must support firstName + email-only contact capture without requiring account creation.
- finalizePrequalQuoteV2 must receive sufficient quote inputs to deterministically finalize pricing, either:
  - from request payload, or
  - from a persisted prequalification request if/when that capability exists
...
- Public schedule UI must use scheduleFirstInspectionPublicV2 as the primary path.
- Temporary V1 fallback is allowed only for platform/deployment failures, never for business-rule failures, and should be considered transitional.
...
- idempotent duplicate-submit behavior for exact same appointment request

DUPLICATE-SUBMIT INVARIANT

For an exact duplicate first-scheduling submit:
- same lead
- same date
- same time slot / same normalized effective time window

scheduleFirstInspectionPublicV2 must:
...
- the current replacement/reschedule behavior may continue
- a new confirmation email may be sent
- the previous active appointment may be cancelled/replaced
- but only one active appointment may remain after completion
...
- SFI_V2_IDEMPOTENCY_CHECK_FAILED
...
- FC ratios + CC breakpoint fields + multiplier present
- target ranges: pH / TA / CH / CYA present
- LSI bounds + TF table present
- retest timetable + revisit policy fields present
...
Usage input policy:
If usage frequency is not collected in the quote wizard, it must be recorded explicitly as "unknown" and the pricing token must be 0 only if "unknown" is a defined option in the pricing-input schema.
Production must never silently assume a usage value.
If "unknown" is not present, release readiness must block with MISSING_INPUT_USAGE.
...
- Photos
- Notes
- Small button: Download Full Service Report
